...
**特别需要注意:** 在使用的过程中, img 应放在单独的文件夹, 例如: image/ 文件夹必须只能有截图的图片, 否则在输出 html 文件的时候会报错, 即使报错, 也无需担心, 只需要运行 HtmlOutput.exe 重新输出 html 文件即可, 这个单独的exe只会生成 html 文件. 并不会有其他的操作（比如不会重新截图）, 但在运行的时候, 需要检查 image/ 文件夹下是否还有其他不相关的文件, 比如 txt 文件和不相关的 img
